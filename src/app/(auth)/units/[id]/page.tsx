...
export default function UnitDetailsPage() {
  ...
  // User needs either canViewAllUnits OR canRead access
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-600">
        ...
        <ChevronRight className="h-4 w-4" />
        <span className="text-gray-900 font-medium">Appartement {unit.unitNumber}</span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900">Appartement {unit.unitNumber}</h1>
          </div>
          {unit.property && (
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="flex flex-col items-center p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-gray-900">{unit.surface || 'N/A'}</div>
                ...
                <div className="flex flex-col items-center p-4 bg-purple-50 rounded-lg">
                  ...
                  <div className="text-xs text-gray-500 mt-1">Salles de bain</div>
                </div>
              ...
              <div className="space-y-3 pt-4 border-t">
                <div className="flex justify-between">
                  <span className="text-gray-600">Type du bien</span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  {unit.amenities.includes('salle_sport') && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-700">Salle de sport</span>
                    </div>
                  )}
                  {unit.amenities.includes('gardiennage') && (
                    <div className="flex items-center gap-2">
                      <Shield className="h-5 w-5 text-blue-600" />
                      <span className="text-gray-700">Gardiennage</span>
                    </div>
                  )}
            ...
            <CardContent className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge className={isOccupied ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}>
                ...
                <div className="flex items-center justify-between pt-2 border-t">
                  <span className="text-sm text-gray-600">Paiement du mois</span>
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-2 pt-2 border-t">
                  {unit.tenant.phone && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-700">{unit.tenant.phone}</span>
                    </div>
                  )}
                  {unit.tenant.email && (
                    <div className="flex items-center gap-2 text-sm">
                      <Mail className="h-4 w-4 text-gray-400" />
                      <span className="text-gray-700">{unit.tenant.email}</span>
                    </div>
                  )}
                  {unit.lease && unit.lease.endDate && (
                    <div className="flex items-center gap-2 text-sm">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900">{unit.owner.name}</p>
                    <p className="text-xs text-gray-500">Propriétaire depuis 2020</p>
                  </div>
                </div>
                {unit.owner.phone && (
                  <div className="flex items-center gap-2 text-sm">
                    <Phone className="h-4 w-4 text-gray-400" />
                    <span className="text-gray-700">{unit.owner.phone}</span>
                  </div>
                )}
                {unit.owner.email && (
                  <div className="flex items-center gap-2 text-sm">
                    <Mail className="h-4 w-4 text-gray-400" />
                    <span className="text-gray-700">{unit.owner.email}</span>
                  </div>
                )}
              </CardContent>
            </Card>
                  ...
                  {unit.recentPayments.slice(0, 3).map((payment: any) => {
                    ...
                    return (
                      <div key={payment.id} className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-gray-900 capitalize">{month} {year}</p>
