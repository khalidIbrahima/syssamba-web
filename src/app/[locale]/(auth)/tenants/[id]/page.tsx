...
export default function TenantProfilePage() {
  ...
  const getPaymentStatus = (payment: any) => {
    ...
    return { label: 'En attente', color: 'text-muted-foreground', bgColor: 'bg-gray-50', dotColor: 'bg-gray-500' };
  };
  ...
  const canEdit = canAccessObject('Tenant', 'edit');
  ...
  return (
    ...
    >
      <PermissionGate
        ...
        action="read"
        showDenied={true}
        deniedMessage="Vous n'avez pas la permission de voir les détails de ce locataire."
      >
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-muted-foreground">Téléphone: {tenant.phone || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Mail className="h-4 w-4 text-muted-foreground" />
                      <span className="text-muted-foreground">Email: {tenant.email || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Briefcase className="h-4 w-4 text-muted-foreground" />
                      <span className="text-muted-foreground">Profession: Ingénieur Informatique</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Calendar className="h-4 w-4 text-muted-foreground" />
                      <span className="text-muted-foreground">Date de naissance: 15/03/1985</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-8">
                <div className="relative w-32 h-32">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-3xl font-bold text-foreground">{cibScore.score}</span>
                    <span className="text-sm text-muted-foreground">{cibScore.label}</span>
                  </div>
                </div>
                <div className="flex-1 space-y-3">
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-muted-foreground">Historique paiements</span>
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-muted-foreground">Stabilité revenus</span>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="text-center py-8 text-muted-foreground">
                  <Handshake className="h-12 w-12 mx-auto mb-2 text-muted-foreground" />
                  <p className="text-sm">Aucun garant enregistré</p>
                  <p className="text-xs text-muted-foreground mt-1">Cette fonctionnalité sera disponible prochainement</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                <p className="text-sm text-muted-foreground mb-1">Bien loué</p>
                <p className="font-medium text-foreground">
                  {tenant.unitNumber} - {tenant.propertyName}
                </p>
                <p className="text-sm text-muted-foreground">Rue 15, Villa 123</p>
              </div>
              <div className="pt-4 border-t space-y-3">
                ...
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Caution versée</span>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-foreground">Renouvellement bail</p>
                  <p className="text-xs text-muted-foreground">Échéance: 31/12/2024</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <input type="checkbox" className="mt-1" />
                <div className="flex-1">
                  <p className="text-sm font-medium text-foreground">Révision loyer annuelle</p>
                  <p className="text-xs text-muted-foreground">Échéance: 01/01/2025</p>
                </div>
              </div>
            </CardContent>
          </Card>
